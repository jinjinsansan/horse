...
export class Spat4Voter {
  ...
  async vote(request: Spat4BetRequest): Promise<Spat4VoteResult> {
    const page = this.ensurePage();
    try {
  ...
  private async selectRace(joName: string, raceNo: number) {
    ...
    const success = await select.selectOption({ label }).catch(async () => {
      const value = await select.evaluate((element: unknown, target: string) => {
        const selectElement = element as { options?: Array<{ value?: string | null; textContent?: string | null }> };
        const options = Array.from(selectElement?.options ?? []);
        const match = options.find((opt) => String(opt?.textContent ?? '').includes(target));
